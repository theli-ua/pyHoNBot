...
def tr(phenny, context): 
   ...
   m = tr.parse.match(context.groups()[1])
   if not m:return
   input,output,phrase = m.groups()
...
tr.commands = ['tr']
#tr.rule = ur'tr (?:([a-z]{2}) +)?(?:([a-z]{2}) +)?["“](.+?)["”]\? *$'
tr.example = '.tr mon chien'
tr.priority = 'low'

def mangle(phenny, input): 
   phrase = input.group(2).encode('utf-8')
   for lang in ['fr', 'de', 'es', 'it', 'ja','ru']: 
      backup = phrase
...
def setup(bot):
    tr.parse = re.compile('(?:([a-z]{2}) +)?(?:([a-z]{2}) +)?["“](.+?)["”]*$')
