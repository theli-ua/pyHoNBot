...
def weather(self, input): 
   ...
   if not icao_code: 
      return self.say( 'Try .weather London, for example?')

   icao_code = code(self, icao_code)

   if not icao_code: 
      self.say( 'No ICAO code found, sorry')
      return
   ...
   if 'Not Found' in bytes: 
      self.say( icao_code+': no such ICAO code, or no NOAA data')
      return
   ...
   if metar[0] == 'AUTO': 
      metar = metar[1:]
   if metar[0] == 'VCU': 
      self.say( icao_code + ': no data provided')
      return
